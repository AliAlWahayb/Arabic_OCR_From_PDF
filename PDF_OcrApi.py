...
def process_page(page, index):
    ...
    try:
        ...
        next_hour = (current_time.replace(minute=0, second=0, microsecond=0) + timedelta(hours=1)).strftime('%Y-%m-%d %H:%M:%S')
        print(f"API rate limit reached. Waiting until the next hour at {next_hour}...")
        time.sleep((current_time.replace(minute=0, second=0, microsecond=0) + timedelta(hours=1) - current_time).total_seconds())
        print("Resuming extraction...")
        ocr_text = api.ocr_file(page)
...
def split_and_process_pdf(pdf_file_path):
    with open(pdf_file_path, 'rb') as pdf_file:
        ...
        for page_num in range(total_pages):
            ...
            output_pdf_path = f"temp_PDF.pdf"  # Define the path to save the current page
            with open(output_pdf_path, 'wb') as output_pdf_file:
                pdf_writer.write(output_pdf_file)

            # Process the current page
            process_page(output_pdf_path, page_num)
...
pdf_file_path = 'PDF/المقامر رواية لـ دوستويفسكي.pdf'
